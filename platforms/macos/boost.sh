#!/bin/bash
#===============================================================================
# I borrowed the script from this repository: 
# https://github.com/faithfracture/Apple-Boost-BuildScript
# Few modifications were made:
#   - build both static and shared libs (macOS only),
#   - build with all libraries except python.
# This is how I ran this script:
# ./boost.sh -macos -j 8 --no-framework
# below this line starts the original comment section.
#===============================================================================
# Filename:  boost.sh
...
bootstrapBoost()
{
    ...
    if [[ -z $BOOST_LIBS ]]; then
        ...
        echo "Bootstrapping for $1 (with libs $BOOST_LIBS_COMMA but no python)"
...
buildBoost_macOS()
{
    ...
    ./b2 $THREADS --build-dir=macos-build --stagedir=macos-build/stage toolset=clang \
        ...
        linkflags="-stdlib=libc++" link=shared,static threading=multi \
